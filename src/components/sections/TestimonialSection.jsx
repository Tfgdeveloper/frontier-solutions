...
export default function TestimonialSlider() {
  ...
  return (
    <section className="py-24 px-6 md:px-12 transition-colors duration-1000 overflow-hidden bg-transprent">
      <div className="max-w-[1600px] mx-auto">
        {/* ── HEADER ── */}
        <div className="flex flex-col lg:flex-row justify-between items-end mb-16 gap-8">
          <div className="max-w-4xl text-left">
            <h2 className="text-4xl md:text-[4rem] font-bold tracking-tight leading-[1.1] text-white">
          ...
          {testimonials.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`testimonial-card w-[300px] md:w-[420px] p-8 rounded-[28px] flex flex-col justify-between shadow-xl bg-[#000] text-white ${
                  ...
                  >
                    {[...Array(item.rating)].map((_, i) => (
                      <motion.div key={i} variants={starItem}>
                        <Star size={14} fill="#fff" stroke="none" className="opacity-80" />
                      </motion.div>
                    ))}
                  </motion.div>

                  <h4 className="font-bold text-lg mb-3 leading-tight text-left">{item.title}</h4>
                  <p className="text-sm leading-relaxed mb-10 text-left text-gray-400">
                    {item.body}
                  </p>
                </div>
          ...
          {testimonials.map((_, i) => (
            <button
              ...
              className={`transition-all duration-300 rounded-full ${
                activeIndex === i ? "w-8 h-2 bg-orange-500" : "w-2 h-2 bg-white/20 hover:bg-white/40"
              }`}
