...
const PricingCard = ({ title, price, desc, features, isSpecial, dark }) => {
  // LOGIC FOR CARDS 1 & 2
  const normalCardStyles = dark 
    ? "bg-[#0A0A0B] text-white border-white/5" 
    : "bg-white text-[#262262] border-black/5 shadow-xl shadow-indigo-900/5";
    
  const normalButtonStyles = dark
  ...
  const specialCardStyles = dark
    ? "bg-white text-[#0A0A0B] border-transparent shadow-2xl" 
    : "bg-[#0A0A0B] text-white border-white/10";
    
  const specialButtonStyles = dark
    ? "bg-[linear-gradient(135deg,_#1F1A4A_0%,_#8C4343_50%,_#E0531A_100%)] text-white shadow-[0_10px_25px_rgba(224,83,26,0.25)] hover:shadow-[0_15px_35px_rgba(224,83,26,0.45)] "
    : "bg-[linear-gradient(135deg,_#1F1A4A_0%,_#8C4343_50%,_#E0531A_100%)] text-white shadow-[0_10px_25px_rgba(224,83,26,0.25)] hover:shadow-[0_15px_35px_rgba(224,83,26,0.45)] ";
...
export default function PricingSection({ dark }) {
  return (
    <section className={`py-40 px-6 md:px-12 transition-colors duration-1000 ${
      dark ? "bg-[#0A0A0B]" : "bg-[#F3F4F6]"
    }`}>
      <div className="max-w-[1400px] mx-auto">
        
        <div className="mb-20 text-center md:text-left">
          <h2 className={`text-5xl md:text-[4rem] font-black tracking-tighter leading-[0.85] mb-4 ${dark ? "text-white" : "text-[#262262]"}`}>
            One ecosystem.<br /> Infinite <span className="text-[#F06A22]">Growth.</span>
          </h2>
          <p className={`max-w-2xl text-lg leading-relaxed ${dark ? "text-white/40" : "text-[#262262]/60"}`}>
